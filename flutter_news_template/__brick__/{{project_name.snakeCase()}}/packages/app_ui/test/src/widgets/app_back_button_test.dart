...
void main() {
  group('AppBackButton', () {
    ...
    testWidgets('renders IconButton when light', (tester) async {
      await tester.pumpApp(
        Scaffold(appBar: AppBar(leading: const AppBackButton.light())),
      );
    ...
    group('navigates', () {
      testWidgets('back when press the icon button', (tester) async {
        ...
        await tester.pumpApp(const AppBackButton(), navigator: navigator);
      ...
      testWidgets('call onPressed when is provided ', (tester) async {
        ...
        await tester.pumpApp(AppBackButton(onPressed: onPressed.call));
        await tester.tap(find.byType(IconButton));
        await tester.pumpAndSettle();
        verify(onPressed.call).called(1);
      });

      testWidgets('call onPressed when is provided '
          'and style is light', (tester) async {
        final onPressed = MockFunction();
        await tester.pumpApp(AppBackButton.light(onPressed: onPressed.call));
        await tester.tap(find.byType(IconButton));
        await tester.pumpAndSettle();
        verify(onPressed.call).called(1);
      });
